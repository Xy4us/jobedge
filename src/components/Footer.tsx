      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-16 pb-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-10 mb-14">
            ...
            <Link href="/" className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-lg bg-[#FF6B00] flex items-center justify-center shadow-sm">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M3 8 C3 5 5 3 8 3 C11 3 13 5 13 8 M5 12 C5 10 6.5 9 8 9 C9.5 9 11 10 11 12" stroke="white" strokeWidth="2" strokeLinecap="round" fill="none"/>
                </svg>
              </div>
              <span className="text-[16px] font-bold text-white tracking-tight">Filo</span>
            </Link>
            <p className="text-[14px] text-white/40 leading-relaxed max-w-[200px] mb-5">
              AI-powered sales platform that helps teams close more deals, faster.
            </p>
            {/* Socials */}
          ...
          {Object.entries(footerLinks).map(([group, links]) => (
            <div key={group}>
              <h4 className="text-[12px] font-semibold text-white/30 tracking-[0.12em] uppercase mb-4">{group}</h4>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-[14px] text-white/60 hover:text-white transition-colors duration-150">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="border-t border-white/10 pt-7 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-[13px] text-white/30">
            © {new Date().getFullYear()} Filo, Inc. All rights reserved.
          </p>
